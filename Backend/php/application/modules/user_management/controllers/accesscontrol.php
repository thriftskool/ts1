...
class accesscontrol extends AdminPanel_Controller {
    ...
    function __construct() {
        ...
        $this->export_tpl = 'adminpanel/accesscontrol/exportuserdata';
...
//        $this->set_message();
    ...
    function downloadcsv() {
        ...
        $university = $this->input->get_post('universitylist');
        $this->form_validation->set_rules('universitylist[]', 'Options', 'required');
        if ($this->form_validation->run() == FALSE) {
            $University_Combo = $this->module_model->University_Combo1($row['fk_university']);
            $this->viewData['University_Combo'] = $University_Combo;
            $this->viewData['accesscontrolTab'] = true;
            $this->viewData['message'] = "Please select atleast one university";
            $this->viewData['adminContentPanel'] = $this->export_tpl;
            $this->load_view();
        } else {
            $filename = 'userdetail';
            $fp = fopen('./upimages/csv/' . $filename . '.csv', 'w');
            $this->load->helper('csv_helper');
            if ($university[0] == '0') {
                $this->load->dbutil();
                $this->load->helper('file');
                $query = $this->db->query("SELECT um.var_emailid,um.var_username,uni.var_name as university_name from users_manage as um LEFT JOIN university_management as uni on um.fk_university = uni.int_glcode where um.chr_delete='N' order by var_username asc");
//                $data = $this->dbutil->csv_from_result($query);
                foreach ($query->result() as $dataset) {
                    $data[$i]['Username'] = $dataset->var_username;
                    $data[$i]['User Email'] = $dataset->var_emailid;
                    $data[$i]['University'] = $dataset->university_name;
                    $i++;
                }
                foreach ($data as $dataset1) {
                    $datatocsv[$i] = $dataset1;
                    $i++;
                }
                fputcsv($fp, array_keys($data[0]));
                foreach ($datatocsv as $fields) {
                    fputcsv($fp, $fields);
                }
                fclose($fp);
                ...
                readfile('./upimages/csv/' . $filename . '.csv');
                exit;
            } else {
                $i = 0;
                $data = array();
                foreach ($university as $unidetail) {
                    $query = $this->db->query("SELECT um.var_emailid,um.var_username,uni.var_name as university_name from users_manage as um LEFT JOIN university_management as uni on um.fk_university = " . $unidetail . " where uni.int_glcode = " . $unidetail . " and um.chr_delete='N' order by var_username asc");
                    foreach ($query->result() as $dataset) {
                        $data[$i]['Username'] = $dataset->var_username;
                        $data[$i]['User Email'] = $dataset->var_emailid;
                        $data[$i]['University'] = $dataset->university_name;
            ...
            readfile('./upimages/csv/' . $filename . '.csv');
            exit;
//            force_download($filename, $datatocsv);
            $University_Combo = $this->module_model->University_Combo1($row['fk_university']);
            $this->viewData['University_Combo'] = $University_Combo;
            $this->viewData['accesscontrolTab'] = true;
            $this->viewData['message'] = "Please find the csv file into following folder upimages/csv";
            $this->viewData['adminContentPanel'] = $this->export_tpl;
            $this->load_view();
        }
    }
    ...
    function exportcsv() {
        ...
        $this->viewData['adminContentPanel'] = $this->export_tpl;
        $this->load_view();
//        $this->load->view('adminpanel/accesscontrol/');
//        $this->load->dbutil();
//        $this->load->helper('file');
//        $query = $this->db->query("SELECT um.var_emailid,uni.var_name as university_name from users_manage as um LEFT JOIN university_management as uni on um.fk_university = uni.int_glcode where um.chr_delete='N' order by var_username asc");
//        $data = $this->dbutil->csv_from_result($query);
//        echo '<pre>';
//        print_r($data);
//        exit;
//        $filename = 'useremailcsv.csv';
//        $this->load->helper('csv_helper');
//        header('Content-type: application/csv');
//        header('Content-Disposition: attachment; filename=' . $filename);
//        header("Content-Transfer-Encoding: UTF-8");
//        $fp = fopen('./upimages/csv/'.$filename .'.csv', 'w');
//        fputcsv($fp, "User Email");
//        
//        fputcsv($fp, array_keys($datasetcsv[0]));
//        foreach ($datatocsv as $fields) {
//            fputcsv($fp, $fields);
//        }
//        $this->index();
