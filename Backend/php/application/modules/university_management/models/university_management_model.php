...
class university_management_model extends CI_Model {
    ...
    var $orderby = 'var_name'; // Attribute of Deafult Order By
...
//            $combo = $this->search_categorycombo();
        ...
        return array('pageurl' => MODULE_PAGE_NAME,
            'heading' => 'Manage University',
//            'listImage' => 'add-new-user-icon.png',
            'tablename' => DB_PREFIX . 'university_management',
            'position' => $position,
            'actionImage' => 'Add University',
//            'actionImageHover' => 'add-new-button-blue-hover.gif',
    ...
    function selectAll() {
        ...
        if (!empty($type)) {
            if ($type == 'autosearch') {
                $orderby = (isset($this->orderby)) ? 'order by ' . $this->orderby . ' ' . $this->ordertype : 'order by var_name desc';
                ...
                $autoSearchQry = $this->db->query("select {$this->searchbyVal} as AutoVal  FROM " . DB_PREFIX . "university_management where {$this->searchbyVal} $searchbyVal  GROUP BY var_name $orderby");
        ...
        $query = $this->db->query("select * ,(SELECT count(1)as totalpages FROM " . DB_PREFIX . "university_management)as totalpages from " . DB_PREFIX . "university_management where chr_delete = 'N' $whereclauseids  $orderby $limitby");
...
//         echo $this->db->last_query(); die;
    ...
    function CountRows() {
        ...
        $sqlCountPages = "select * FROM " . DB_PREFIX . "university_management  where chr_delete='N' $whereclauseids";
    ...
    function select_Rows($id) {
        ...
        $query = $this->db->get_where('university_management', array('int_glcode' => $id));
        $row = $query->row_array();
        return $row;
    }
    ...
    public function insert() {
        if ($_FILES['var_image']['name'] != '') {
            ...
            $maindir = 'upimages/university_management/images/';

            $thumbdir = 'upimages/university_management/thumb/';
		...
		$updateSql = "SELECT MAX(int_glcode) FROM university_management";
		$res = $this->db->query($updateSql);
		$new_id =  (int)$res->row_array()['max'] + 1;
		$data = array(
			'int_glcode' => $new_id,
            'var_name' => $this->input->post('var_name', true),
            'var_image' => $var_thumb,
            'var_domain' => $this->input->post('var_domain'),
		...
		$this->db->insert('university_management', $data);
        return $new_id;
    }
    ...
    function update() {
        ...
        $this->db->update('university_management', $data);
    }
...
//        $this->db->cache_set_common_path("application/cache/db/common/special/");
        ...
        $this->mylibrary->update_display_order1($uids, $neworder, $oldorder, '', 'university_management', $where);
        $tablename = DB_PREFIX . 'university_management';
...
//            $this->mylibrary->insertinlogmanager(MODULE_ID, DB_PREFIX . 'university_management', 'var_title', $deletearray[$i], 'D', 'int_glcode');
        }
    }
