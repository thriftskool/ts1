...
class Adminpanel_university_management extends AdminPanel_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('university_management_model', 'module_model');                       // MODULE MODEL

        $this->main_tpl = 'adminpanel/university_management_tpl';                 // MODULE MAIN VIEW
        $this->add_tpl = 'adminpanel/university_management_add_tpl';              // MODULE ADD  VIEW
        $this->module_url = base_url() . 'adminpanel/university_management/';          // MODULE URL    
...
//        {
//            $fk_category = $_REQUEST['c_id'];
//        }
//        else   {
            $fk_category = $row['fk_category'];
//        }
        $data_category_comb = $this->module_model->getcategorycombo('fk_category',$eid,$fk_category,(!empty($row['fk_category']) ? $row['fk_category'] : ''), 'add-new-user-textarea2', '1');
        $this->viewData['categorycombo'] = $data_category_comb;
    ...
    public function insert() {
        ...
        $this->form_validation->set_rules('var_name', 'Title', 'trim|required');
    ...
    public function update() {
        ...
        if($id=='1'){
           $this->session->set_flashdata('msg', 'notedited');
            ...
            $eid = $this->input->get_post('eid');
            redirect(base_url() . 'adminpanel/university_management/add?' . 'eid=' . $eid);
        }
        else{
