...
//  *******************************  New End  ******************************************
                ...
                if ($Result['chr_verify'] == 'N') {
                    ...
                    $response['Response'] = array(
                        ...
                        'profileimgpath' => SITE_PATH . 'upimages/profilepic/',
	...
	function Get_Login_Email($id, $pwd, $device_id) {
        ...
        if ($username != '' && $password != '') {
			...
			$cond = "var_emailid ILIKE '" . $username . "' and var_password = '" . $this->mylibrary->cryptPass($password) . "' and chr_delete = 'N'";
            $this->db->where($cond);
            $Query = $this->db->get('users_manage');
            $Result = $Query->row_array();
            $Count = $Query->num_rows();
...
//    }
    ...
    function Get_Registration($uni_id, $email_id, $username, $password, $device_id, $personname, $class, $profileimg) {
        $i = 1;
        $this->Check_Email($email_id);
        $this->Check_Username($username);

        if ($profileimg != '' || $profileimg != NULL) {
            $imageData = base64_decode($profileimg);
            $source = imagecreatefromstring($imageData);
            $filename = $username . "_" . $i . '.jpeg';
            $file = 'upimages/profilepic/' . $filename;
            imagejpeg($source, $file, 100);
        } else {
            $filename='default1324657089.png';
        }
        //echo "test=".$username;
        if($personname == ''){
...
//    Update_Edit_Profile
    ...
    function Update_Edit_Profile($user_id, $university_id, $email_id, $var_username, $personname, $class, $profileimg) {
        ...
        $i = 1;
        $source = imagecreatefromstring($imageData);
        $filename = $var_username . "_" . $i . '.jpeg';
        $file = 'upimages/profilepic/' . $filename;
        imagejpeg($source, $file, 100);
        if (!empty($Result_user_check)) {
            if ($Result_user_check['chr_publish'] == 'Y') {
...
//             echo $this->db->last_query(); die;
    ...
    function get_class() {
        $query = $this->db->query("SELECT * from tbl_class order by int_glcode");
    ...
    function post_owner_detail($owner_id) {
//        $query = $this->db->query("select ow.var_username,ow.profile_img,ow.person_name,ow.class,owp.* from users_manage ow LEFT JOIN post_management owp on ow.int_glcode = owp.fk_user_id where ow.int_glcode = ".$owner_id);
        $query = $this->db->query("select ow.var_username,ow.profile_img,ow.person_name,cl.class_name from users_manage ow LEFT JOIN tbl_class cl on ow.class:: integer = cl.int_glcode where ow.int_glcode = " . $owner_id);
        $data = $query->result_array();
        //print_r($data);
        $query1 = $this->db->query("select owp.int_glcode as post_id,"
                . "owp.fk_user_id as user_id,"
                . "owp.var_postname as post_name,"
                . "owp.var_description as description,"
                . "owp.var_price as price,"
                . "owp.dt_expiredate as expiredate,"
                . "owp.chr_status as status,"
                . "fv.chr_favorite as favorite,"
                . "(SELECT pg.var_image FROM post_gallery as pg WHERE pg.fk_post = owp.int_glcode OFFSET 0 limit 1) AS image, "
                . "(SELECT pg.int_glcode FROM post_gallery as pg WHERE pg.fk_post = owp.int_glcode OFFSET 0 limit 1) AS image_id "
                . "FROM post_management as owp "
                . "LEFT JOIN favorite as fv on owp.int_glcode=fv.fk_post "
                . "where owp.fk_user_id = " . $owner_id);
        $data1 = $query1->result_array();
        $response_cate_list['Response'] = array(
            'status' => '200',
            'message' => 'Success.',
            'imagepath' => SITE_PATH . 'upimages/profilepic/',
            'postimgpath' => SITE_PATH . 'upimages/post_gallery/',
            'var_username' => $data[0]['var_username'],
