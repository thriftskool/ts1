...
//            $combo = $this->search_categorycombo();
        ...
        return array('pageurl' => MODULE_PAGE_NAME,
            'heading' => 'Manage Post Category',
//            'listImage' => 'add-new-user-icon.png',
            'tablename' => DB_PREFIX . 'post_category',
            'position' => $position,
...
//            'actionImageHover' => 'add-new-button-blue-hover.gif',
            ...
            'search' => array('searchArray' => array("var_categoryname" => "Title"),
    ...
    function selectAll() {
        ...
        if (trim($this->searchtxt) != '' || $this->filterby != '0') {
            if ($this->searchtxt != '') {
                $whereclauseids .= (empty($this->searchby)) ? " and var_categoryname ILIKE '%" . addslashes(htmlspecialchars_decode($this->searchtxt)) . "%'" : " and $this->searchby ILIKE '%" . addslashes(htmlspecialchars_decode($this->searchtxt)) . "%'";
            }
            if ($this->filterby != '0') {
...
//                echo $searchbyVal; die;
                if ($this->searchbyVal == '0' || $this->searchbyVal == '') {
                    $this->searchbyVal = "var_categoryname";
                }
               $autoSearchQry = $this->db->query("select {$this->searchbyVal} as AutoVal  FROM " . DB_PREFIX . "post_category where {$this->searchbyVal} $searchbyVal  GROUP BY var_categoryname $orderby");
               $this->mylibrary->GetAutoSearch($autoSearchQry);
            }
        }
        $limitby = 'OFFSET ' . $this->start . ' LIMIT ' . $this->pagesize;
        $orderby = 'order by int_glcode asc';
        $query = $this->db->query("select * ,(SELECT count(1)as totalpages FROM " . DB_PREFIX . "post_category)as totalpages from " . DB_PREFIX . "post_category where chr_delete = 'N' $whereclauseids  $orderby $limitby");
//      echo $this->db->last_query();
    ...
    function CountRows() {
        ...
        $sqlCountPages = "select * FROM " . DB_PREFIX . "post_category  where chr_delete='N' $whereclauseids";
    ...
    function select_Rows($id) {
        ...
        $query = $this->db->get_where('post_category', array('int_glcode' => $id));
        $row = $query->row_array();
        return $row;
    }
...
if ($_FILES['var_image']['name'] != '') {
            ...
            $maindir = 'upimages/post_category/images/';
            $thumbdir = 'upimages/post_category/thumb/';
...
//        $this->db->cache_set_common_path("application/cache/db/common/special/");
        ...
        $this->mylibrary->update_display_order1($uids, $neworder, $oldorder, '', 'post_category', $where);
        $tablename = DB_PREFIX . 'post_category';
...
//            $this->mylibrary->insertinlogmanager(MODULE_ID, DB_PREFIX . 'post_category', 'var_title', $deletearray[$i], 'D', 'int_glcode');
        }
    }
