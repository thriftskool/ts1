...
class Adminpanel_job_management extends AdminPanel_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('job_management_model', 'module_model');                       // MODULE MODEL

        $this->main_tpl = 'adminpanel/job_management_tpl';                 // MODULE MAIN VIEW
        $this->add_tpl = 'adminpanel/job_management_add_tpl';              // MODULE ADD  VIEW
        $this->module_url = base_url() . 'adminpanel/job_management/';          // MODULE URL    
    }
    ...
    public function insert() {
        ...
        $this->form_validation->set_rules('var_jobname', 'Title', 'trim|required');
    ...
    public function update() {
        ...
        if ($this->form_validation->run($this) == FALSE) {
            ...
            if ($id == '1') {
                ...
                $eid = $this->input->get_post('eid');
                redirect(base_url() . 'adminpanel/job_management/add?' . 'eid=' . $eid);
            } else {
