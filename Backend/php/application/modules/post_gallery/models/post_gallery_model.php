...
//        echo $this->input->get_post('pid'); die;
//        print_r($_REQUEST);
//        die;
...
//        $this->pid = (!empty($pid)) ? $pid : $this->pid;
...
//        die;
        ...
        $this->addpagename = MODULE_PAGE_NAME . '/add?pid=' . $this->input->get_post('pid');

        $this->hurlwithpara = $this->pagename . '&' . 'hpagesize=' . $this->pagesize . '&hnumofrows=' . $this->numofrows . '&horderby=' . $this->orderby . '&hordertype=' . $this->ordertype . '&hsearchby=' . $this->searchby . '&hsearchtxt=' . $this->searchtxt . '&hpagenumber=' . $this->pagenumber . '&hfilterby=' . $this->filterby . '&history=T' . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->urlwithpara = $this->pagename . '&' . 'pagesize=' . $this->pagesize . '&numofrows=' . $this->numofrows . '&orderby=' . $this->orderby . '&ordertype=' . $this->ordertype . '&searchby=' . $this->searchby . '&searchtxt=' . $this->searchtxt . '&pagenumber=' . $this->pagenumber . '&filterby=' . $this->filterby . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->urlwithpoutsearch = $this->pagename . '&' . 'pagesize=' . $this->pagesize . '&orderby=' . $this->orderby . '&ordertype=' . $this->ordertype . '&filterby=' . $this->filterby . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->urlwithoutsort = $this->pagename . '&' . 'pagesize=' . $this->pagesize . '&numofrows=' . $this->numofrows . '&searchby=' . $this->searchby . '&searchtxt=' . $this->searchtxt . '&pagenumber=' . $this->pagenumber . '&ordertype=' . $this->ordertype . '&filterby=' . $this->filterby . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->urlwithoutpaging = $this->pagename . '&orderby=' . $this->orderby . '&ordertype=' . $this->ordertype . '&searchby=' . $this->searchby . '&searchtxt=' . $this->searchtxt . '&filterby=' . $this->filterby . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->urlwithoutfilter = $this->pagename . '&' . 'pagesize=' . $this->pagesize . '&orderby=' . $this->orderby . '&ordertype=' . $this->ordertype . '&searchby=' . $this->searchby . '&searchtxt=' . $this->searchtxt . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->AutoSearchUrl = $this->urlwithpara . "&type=autosearch&searchbyVal=" . $this->searchbyVal . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->addurlwithpara = $this->addpagename . '&' . 'pagesize=' . $this->pagesize . '&numofrows=' . $this->numofrows . '&orderby=' . $this->orderby . '&ordertype=' . $this->ordertype . '&searchby=' . '&pagenumber=' . $this->pagenumber . '&filterby=' . $this->filterby . '&categoryid=' . $this->categoryid . '&pid=' . $this->input->get_post('pid');
        $this->fronturlwithpara = $this->pagename . '&' . 'pagesize=' . urlencode($this->pagesize) . '&numofrows=' . $this->numofrows . '&categoryid=' . urlencode($this->categoryid) . $Ftr_cat . '&pid=' . $this->input->get_post('pid');
...
//        echo $gi; die;
        return array('pageurl' => MODULE_PAGE_NAME . '?pid=' . $this->input->get_post('pid'),
            'heading' => 'Manage Post Gallery',
//            'listImage' => 'add-new-user-icon.png',
            'tablename' => DB_PREFIX . 'post_gallery',
            'position' => $position,
            'actionImage' => 'Add New Image',
//            'actionImageHover' => 'add-new-button-blue-hover.gif',
            'actionUrl' => MODULE_PAGE_NAME . '/add?pid=' . $this->input->get_post('pid'),
            'dispPaging' => 'Yes',
    ...
    function count_photo($pids) {
        $sql = "select * from post_gallery where chr_delete = 'N' and fk_post= $pids";
    ...
    function selectAll() {
        ...
        if ($pids != '') {
            $whereclauseids .=" and fk_post = '" . $pids . "'";
        }
        $type = $this->input->get_post('type');
        if (!empty($type)) {
            if ($type == 'autosearch') {
...
//                echo $searchbyVal; die;
                ...
                $autoSearchQry = $this->db->query("select {$this->searchbyVal} as AutoVal  FROM " . DB_PREFIX . "post_gallery where {$this->searchbyVal} $searchbyVal  GROUP BY var_title $orderby");
        ...
        $query = $this->db->query("select * ,(SELECT count(1)as totalpages FROM " . DB_PREFIX . "post_gallery)as totalpages from " . DB_PREFIX . "post_gallery where chr_delete = 'N' $whereclauseids  $orderby $limitby");
        return $query;
    }
    ...
    function CountRows() {
        ...
        $sqlCountPages = "select * FROM " . DB_PREFIX . "post_gallery  where chr_delete='N' $whereclauseids";
    ...
    function select_Rows($id) {
        $query = $this->db->get_where('post_gallery', array('int_glcode' => $id));
        $row = $query->row_array();
        return $row;
    }
    ...
    public function insert() {
        ...
        if ($_FILES['var_image']['name'] != '') {
            ...
            $maindir = 'upimages/post_gallery/images/';
            $thumbdir = 'upimages/post_gallery/thumb/';
            ...
            $this->UNIVERSITY_WIDTH = CAMPUS_GALLERY_WIDTH;
            $this->UNIVERSITY_HEIGHT = CAMPUS_GALLERY_HEIGHT;
        ...
        $data = array(
            ...
            'fk_post' => $this->input->post('pid', true),
            'fk_post' => $this->input->post('pid', true),
            'fk_user_id' => '0',
            'var_image' => $var_thumb,
...
//        echo '<pre>';print_r($data);exit;
        $this->db->insert('post_gallery', $data);
    ...
    function update() {
        ...
        $this->db->update('post_gallery', $data);
    }
...
//        $this->db->cache_set_common_path("application/cache/db/common/special/");
        ...
        $this->mylibrary->update_display_order1($uids, $neworder, $oldorder, '', 'post_gallery', $where);
        $tablename = DB_PREFIX . 'post_gallery';
...
//        print_r($deletearray);die;
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            $data = array('chr_delete' => 'Y', 'fk_post' => $pids);
