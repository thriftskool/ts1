...
class university_leads_model extends CI_Model {
    ...
    var $orderby = 'int_glcode'; // Attribute of Deafult Order By
...
//            $combo = $this->search_categorycombo();
        ...
        return array('pageurl' => MODULE_PAGE_NAME,
            'heading' => 'Manage New University',
//            'listImage' => 'add-new-user-icon.png',
            'tablename' => DB_PREFIX . 'university_leads',
            'position' => $position,
//            'actionImage' => 'Add Post Category',
...
//            'actionUrl' => MODULE_PAGE_NAME . '/add',
    ...
    function selectAll() {
        ...
        if (trim($this->searchtxt) != '' || $this->filterby != '0') {
            if ($this->searchtxt != '') {
                $whereclauseids .= (empty($this->searchby)) ? " and var_name ILIKE '%" . addslashes(htmlspecialchars_decode($this->searchtxt)) . "%'" : " and $this->searchby ILIKE '%" . addslashes(htmlspecialchars_decode($this->searchtxt)) . "%'";
            }
            if ($this->filterby != '0') {
        ...
        if (!empty($type)) {
            if ($type == 'autosearch') {
                $orderby = (isset($this->orderby)) ? 'order by ' . $this->orderby . ' ' . $this->ordertype : 'order by int_glcode desc';
...
//                echo $searchbyVal; die;
               ...
               $autoSearchQry = $this->db->query("select {$this->searchbyVal} as AutoVal  FROM " . DB_PREFIX . "university_leads where {$this->searchbyVal} $searchbyVal  GROUP BY var_name $orderby");
               $this->mylibrary->GetAutoSearch($autoSearchQry);
            }
        }
        $limitby = 'OFFSET ' . $this->start . ' LIMIT ' . $this->pagesize;
        $orderby = 'order by int_glcode asc';
        $query = $this->db->query("select * ,(SELECT count(1)as totalpages FROM " . DB_PREFIX . "university_leads)as totalpages from " . DB_PREFIX . "university_leads where chr_delete = 'N' $whereclauseids  $orderby $limitby");
//      echo $this->db->last_query();
    ...
    function CountRows() {
        ...
        $sqlCountPages = "select * FROM " . DB_PREFIX . "university_leads  where chr_delete='N' $whereclauseids";
    ...
    function select_Rows($id) {
        ...
        $query = $this->db->get_where('university_leads', array('int_glcode' => $id));
        $row = $query->row_array();
        return $row;
    }
...
if ($_FILES['var_image']['name'] != '') {
            ...
            $maindir = 'upimages/university_leads/images/';
            $thumbdir = 'upimages/university_leads/thumb/';
            $var_main_file = $this->mylibrary->generate_image('var_image', $maindir);
...
//            image_thumb($maindir . $var_main_file, $this->PRODUCT_WIDTH, $this->PRODUCT_HEIGHT);
            $var_thumb = $var_main_file;
        }
//echo $_FILES['var_backimage']['name'] ; die;
        if ($_FILES['var_backimage']['name'] != '') {
            $photo = basename($_FILES['var_backimage']['name']);
            $photo = str_replace('&', "_", $photo);
            $photo = str_replace(' ', "_", $photo);
            $photofile = preg_replace('/[^a-zA-Z0-9_ \[\]\.\(\)&-]/s', '', $photo);
            $phototitle = explode('.', $photofile);
            ...
            $maindir = 'upimages/university_leads/images/';
            $thumbdir = 'upimages/university_leads/thumb/';
            $var_main_file2 = $this->mylibrary->generate_image('var_backimage', $maindir);

            $file_photo2 = basename($var_main_file2);
            $uploadedfile2 = $maindir . $file_photo2;
//            $this->PRODUCT_WIDTH = PRODUCT_WIDTH;
//            $this->PRODUCT_HEIGHT = PRODUCT_HEIGHT;
//            image_thumb($maindir . $var_main_file, $this->PRODUCT_WIDTH, $this->PRODUCT_HEIGHT);
            $var_thumb2 = $var_main_file2;
        }
        
        $data = array(
        'var_categoryname' => $this->input->post('var_categoryname', true),
        'var_backimage' => $var_thumb2,
        'var_image' => $var_thumb,
...
//            image_thumb($maindir . $var_main_file, $this->PRODUCT_WIDTH, $this->PRODUCT_HEIGHT);
         ...
         if ($var_thumb2 == '') {
            $var_thumb2 = $this->input->get_post('hidvar_backimage');
        }
        ...
        $this->db->update('university_leads', $data);
    }
...
//        $this->db->cache_set_common_path("application/cache/db/common/special/");
        ...
        $this->mylibrary->update_display_order1($uids, $neworder, $oldorder, '', 'university_leads', $where);
        $tablename = DB_PREFIX . 'university_leads';
...
//            $this->mylibrary->insertinlogmanager(MODULE_ID, DB_PREFIX . 'university_leads', 'var_title', $deletearray[$i], 'D', 'int_glcode');
        }
    }
