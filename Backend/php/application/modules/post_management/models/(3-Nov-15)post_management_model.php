...
//            'dt_modifydate' => date('Y-m-d'),
        ...
        $sql_property = "select fk_user_id from post_management WHERE int_glcode='" . $this->input->get_post('ehintglcode') . "'";
        $data = $this->db->query($sql_property);
        $row = $data->row_array();
//echo $row['fk_user_id']; die;
        $sql = "select int_glcode from logmanager where var_referenceid='" . $this->input->get_post('ehintglcode') . "' and var_tablename='post_management'";
        $query = $this->db->query($sql);
        $count = $query->num_rows;
        if ($count == 0) {
            $this->mylibrary->insertinlogmanager(MODULE_ID, 'post_management', $row['fk_user_id'], $this->input->get_post('ehintglcode'), "");
        }
    }
...
//            die;
            ...
            $message = array(
                ...
                "message" => "Your post '" . $Data_Array['post_name'] . "' has been blocked."
            );
//            print "<pre/>";
//            print_r($message); 
//            die;
