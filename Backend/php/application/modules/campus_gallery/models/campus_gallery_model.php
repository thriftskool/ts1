...
class campus_gallery_model extends CI_Model {
...
//            $combo = $this->search_categorycombo();
        ...
        return array('pageurl' => MODULE_PAGE_NAME . '?cid=' . $this->input->get_post('cid'),
            'heading' => 'Manage Deals Gallery',
//            'listImage' => 'add-new-user-icon.png',
            'tablename' => DB_PREFIX . 'campus_gallery',
            'position' => $position,
...
//            'actionImageHover' => 'add-new-button-blue-hover.gif',
    ...
    function selectAll() {
        ...
        if ($cids != '') {
            $whereclauseids .=" and fk_campus_deal = '" . $cids . "'";
        }
        $type = $this->input->get_post('type');
        if (!empty($type)) {
            if ($type == 'autosearch') {
...
//                echo $searchbyVal; die;
                ...
                $autoSearchQry = $this->db->query("select {$this->searchbyVal} as AutoVal  FROM " . DB_PREFIX . "campus_gallery where {$this->searchbyVal} $searchbyVal  GROUP BY var_title $orderby");
        ...
        $query = $this->db->query("select * ,(SELECT count(1)as totalpages FROM " . DB_PREFIX . "campus_gallery)as totalpages from " . DB_PREFIX . "campus_gallery where chr_delete = 'N' $whereclauseids  $orderby $limitby");
        return $query;
    }
    ...
    function CountRows() {
        ...
        $sqlCountPages = "select * FROM " . DB_PREFIX . "campus_gallery  where chr_delete='N' $whereclauseids";
    ...
    function select_Rows($id) {
        $query = $this->db->get_where('campus_gallery', array('int_glcode' => $id));
        $row = $query->row_array();
        return $row;
    }
    ...
    public function insert() {
        ...
        if ($_FILES['var_image']['name'] != '') {
            ...
            $maindir = 'upimages/campus_gallery/images/';
            $thumbdir = 'upimages/campus_gallery/thumb/';
            $var_main_file = $this->mylibrary->generate_image('var_image', $maindir, $idss);
//            echo $var_main_file; die;
        ...
        $data = array(
            ...
            'fk_campus_deal' => $this->input->post('cid', true),
            'fk_user_id' => '0',
            'var_image' => $var_thumb,
...
//        echo '<pre>';print_r($data);exit;
        $this->db->insert('campus_gallery', $data);
    ...
    function update() {
        ...
        $this->db->update('campus_gallery', $data);
    }
...
//        $this->db->cache_set_common_path("application/cache/db/common/special/");
        ...
        $this->mylibrary->update_display_order1($uids, $neworder, $oldorder, '', 'campus_gallery', $where);
        $tablename = DB_PREFIX . 'campus_gallery';
...
//        print_r($deletearray);die;
        ...
        for ($i = 0; $i < $totaldeletedrecords; $i++) {
            $data = array('chr_delete' => 'Y', 'dt_modifydate' => date('Y-m-d'), 'fk_campus_deal' => $cids);
            $this->db->where('int_glcode', $deletearray[$i]);
            $this->db->update($tablename, $data);
//            $this->mylibrary->insertinlogmanager(MODULE_ID, DB_PREFIX . 'campus_gallery', 'var_title', $deletearray[$i], 'D', 'int_glcode');
        }
    }
