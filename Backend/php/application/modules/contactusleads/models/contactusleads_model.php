...
//                echo $searchbyVal; die;
               ...
               $autoSearchQry = $this->db->query("select {$this->searchbyVal} as AutoVal  FROM " . DB_PREFIX . "contactusleads where {$this->searchbyVal} $searchbyVal  GROUP BY var_name $orderby");
               $this->mylibrary->GetAutoSearch($autoSearchQry);
            }
        }
        $limitby = 'OFFSET ' . $this->start . ' LIMIT ' . $this->pagesize;
        $orderby = 'order by int_glcode asc';
        $query = $this->db->query("select * ,(SELECT count(1)as totalpages FROM " . DB_PREFIX . "contactusleads)as totalpages from " . DB_PREFIX . "contactusleads where chr_delete = 'N' $whereclauseids  $orderby $limitby");
//      echo $this->db->last_query();
    ...
    function CountRows() {
        ...
        $sqlCountPages = "select * FROM " . DB_PREFIX . "contactusleads  where chr_delete='N' $whereclauseids";
    ...
    function select_Rows($id) {
        ...
        $query = $this->db->get_where('contactusleads', array('int_glcode' => $id));
        $row = $query->row_array();
        return $row;
    }
...
if ($_FILES['var_image']['name'] != '') {
            ...
            $maindir = 'upimages/contactusleads/images/';
            $thumbdir = 'upimages/contactusleads/thumb/';
...
//            image_thumb($maindir . $var_main_file, $this->PRODUCT_WIDTH, $this->PRODUCT_HEIGHT);
        ...
        $this->db->update('contactusleads', $data);
    }
...
//        $this->db->cache_set_common_path("application/cache/db/common/special/");
        ...
        $this->mylibrary->update_display_order1($uids, $neworder, $oldorder, '', 'contactusleads', $where);
        $tablename = DB_PREFIX . 'contactusleads';
...
//            $this->mylibrary->insertinlogmanager(MODULE_ID, DB_PREFIX . 'contactusleads', 'var_title', $deletearray[$i], 'D', 'int_glcode');
        }
    }
