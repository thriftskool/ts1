...
class Adminpanel_campus_deal extends AdminPanel_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('campus_deal_model', 'module_model');                       // MODULE MODEL

        $this->main_tpl = 'adminpanel/campus_deal_tpl';                 // MODULE MAIN VIEW
        $this->add_tpl = 'adminpanel/campus_deal_add_tpl';              // MODULE ADD  VIEW
        $this->module_url = base_url() . 'adminpanel/campus_deal/';          // MODULE URL    
        $this->view_tpl = 'adminpanel/campus_deal_view_tpl';              // MODULE ADD  VIEW        
    }
    ...
    public function view() {
        ...
        $query = $this->module_model->select_View_deal_data($cid);
//        $this->viewData['counttotal'] = $query->num_rows();
        $this->viewData['select_View_deal_data'] = $query;
    ...
    public function update() {
        ...
        if ($this->form_validation->run($this) == FALSE) {
            ...
            if ($id == '1') {
                ...
                $eid = $this->input->get_post('eid');
                redirect(base_url() . 'adminpanel/campus_deal/add?' . 'eid=' . $eid);
            } else {
