...
//echo "asfdsaf"; die;
        ...
        $this->ci->email->smtp_host = 'ssl://smtp.gmail.com';
        $this->ci->email->smtp_auth = true;
        $this->ci->email->smtp_user = "no-reply@thriftskool.com";
        $this->ci->email->smtp_pass = "Thriftskool22";
        $this->ci->email->smtp_port = "25";
        $this->ci->email->set_mailtype('html');
        if (!$this->ci->email->send()) {
...
// NOTE: PHP will clean up the temp file it created when the request  // has completed.
    ...
    function generate_image_Deal($filefield, $path = '', $idss, $imagename, $tmpname, $size, $type, $j) {
...
//        die;
        $max_file_size = MAX_FILE_SIZE;
        $sess = time();
        $des_file_photo = basename($imagename);
//        echo $des_file_photo; die;
        ...
        if (!empty($idss)) {
            if (!empty($_REQUEST['hidvar_image'])) {
                ...
                $des_file_photo = $idss . '-' . $j . '.' . $lastitem;
            }
        } else {
...
//echo $des_file_photo; die;
        ...
        $source_file = basename($imagename);
        $file = $uploaddir . $des_file_photo;
//        echo $file; die;
...
//        echo $uploadedfile; die;
        $image_info = getimagesize($uploadedfile);
        $imageextension = $type;
        $file_size_MB = number_format($size / pow(1920, 2)); // file size in MB
//       echo $size;
//        echo  $file_size_MB; 
        if ($size < $file_size_MB) {
//            if ($imageextension == "image/pjpeg" || $imageextension == "image/jpeg" || $imageextension == "image/jpeg" || $imageextension == "image/JPG" || $imageextension == "image/jpg") {
...
//            echo $tmpname; die;
            $img_upload = move_uploaded_file($tmpname, $file);
        }
        return $des_file_photo;
    }

    function generate_image_Deal_update($filefield, $path = '', $idss, $imagename, $tmpname, $size, $type, $name1, $name2) {
...
//            } else {
//                $des_file_photo = $idss . '-' . $j . '.' . $lastitem;
            $des_file_photo = $name1;
//            }
//                echo $des_file_photo; die;
        } else {
            $des_file_photo = $idss . '-' . $name2 . '.' . $lastitem;
//            echo $des_file_photo; die;
//            $des_file_photo = $pieces . $sess . uniqid() . '.' . $lastitem;
        }
