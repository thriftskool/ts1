 ...
 */
public class ReplyMessageDetail_AsyncTask extends AsyncTask<String, Integer, JSONObject> {
    ...
    public static String str_ThreadId;



    public ReplyMessageDetail_AsyncTask(Context contex, List<InBoxDetaiList_Been> messageList, AmazingListView lv_listView, TextView tv_title,String str_postName,  List<InBoxDetaiList_Been> messageList1 ,List<Pair<String, List<InBoxDetaiList_Been>>> pairMessageList ) {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        try
        {
            ...
            String response = Utility.postRequest(Comman.URL+"go_to_message_list_from_reply", json.toString(),context);
        ...
        try
        {
            ...
            if (result.getString("status").trim().equalsIgnoreCase("200"))
            {
                ...
                for (int i=0; i<detailArray.length(); i++)
                {
                    ...
                    str_ThreadId=detailItem.getString("thread_id");
                        tv_title.setText(detailItem.getString("own_username").trim()+" - "+str_postName.trim());


                    ReplyMessageDetail_Tab_Activity.str_ThreadId=str_ThreadId;

                    ReplyMessageDetail_Tab_Activity.str_postUserid = detailItem.getString("post_user_id").trim();
                }
...
*/
               ...
               /* messageList.clear();
                ...
                for (int i=0; i<detailArray.length(); i++)
                {
                    ...
                    ReplyMessageDetail_Tab_Activity.str_ThreadId = detailItem.getString("thread_id");
                }



//                adapter =new InBoxDetailList_Adapter(context, R.layout.inbox_detail_list_row,  messageList,tv_title,str_postName );
